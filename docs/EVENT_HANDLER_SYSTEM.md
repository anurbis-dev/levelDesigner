# Руководство по системе обработчиков событий v3.51.9
...
autoEventHandlerManager.registerWindowManually(
    dialogElement,    // элемент окна
    windowInstance,   // экземпляр окна
    'settings',      // тип окна
    'my-dialog'      // ID окна
);
...
EventHandlerUtils.removeDialogEventHandling('my-dialog', eventHandlerManager);
autoEventHandlerManager.unregisterWindow('my-dialog');
```
...
### 2. Очищайте обработчики при удалении элементов

```javascript
// В методе destroy() или hide()
EventHandlerUtils.removeDialogEventHandling('my-dialog', eventHandlerManager);
```
