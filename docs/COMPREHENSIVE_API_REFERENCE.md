# Полный справочник API Level Editor v3.50.3
...
### LevelEditor - Исправление дабл-клика разделителей

#### **Новые методы управления подписками:**
- `this.subscriptions = []` - Массив для хранения подписок StateManager (в конструкторе)
- `setupPanelSizeListeners()` - Настройка слушателей для изменения размеров панелей
- `destroy()` - Отмена всех подписок StateManager перед уничтожением

#### **Технические исправления:**
- **Убрана блокировка событий**: `pointerEvents = 'none'` удалено из mousedown обработчиков
- **Правильная инициализация**: setupPanelResizing() вызывается после создания window.editor
- **Управление памятью**: Подписки StateManager сохраняются и отменяются в destroy()
- **Логирование**: Logger.layout.info при двойном клике по разделителям

### CSS Styles - Исправления разделителей v3.50.2
- **`.resizer`, `.resizer-x`**: Вертикальные разделители с корректными hover индикаторами
- **`.resizer-y`**: Горизонтальные разделители с исправленной ориентацией
- **`.resizing`**: Активное состояние для всех разделителей
- **AssetPanel**: Обновлен JavaScript для использования CSS классов вместо inline стилей
