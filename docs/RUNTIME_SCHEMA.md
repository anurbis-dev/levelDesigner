...
| `levelData.inputMap` | Keyboard action→keys `{actions: {name: string[]}}` (§7 `inputMap` Tier 3); null/absent = `Input.DEFAULT_ACTIONS` |
...
| `inputMap` | Input Map | 1 | partial (Tier 3: level-scope keyboard remap) | Not a catalog asset — level field `levelData.inputMap` `{actions: {actionName: string[]}}`. `Input.setInputMap` / `isActionDown`; `GameEngine.loadProject` applies map; movement axes + `interact` (Event Graph `OnInteract`, `MountableVehicleSeatBehavior`). Unmapped actions fall back to `Input.DEFAULT_ACTIONS` (WASD/arrows + `e`). Gamepad/touch and editor authoring UI not in this pass. |
...
Источник: `COMPONENT_TYPES` в `src/constants/ComponentTypes.js`, 21 id. Статус по каждому — в
...
| `pathFollower` | Path Follower | 1 | implemented | `waypoints` (JSON `[{x,y},…]`, offsets from entity spawn position — same asset-local convention as collider freeform `points`; conventionally `waypoints[0] = {x:0,y:0}`), `speed` (px/sec, дефолт 100), `mode` (`loop`\|`pingpong`\|`once`, дефолт `loop`), `waitAtWaypoint` (сек, дефолт 0), `interpolation` (`linear`\|`smooth`, дефолт `linear` — §7 `pathSpline` Tier 1). Самодостаточное кинематическое поведение: каждый тик двигает entity.x/y к текущей целевой waypoint на `speed`; по достижении — пауза `waitAtWaypoint`, затем переход к следующей согласно `mode` (`loop` — wrap к первой, `pingpong` — реверс на концах, `once` — останов на последней). `interpolation:'smooth'` двигает entity по кривой Catmull-Rom через `waypoints` вместо прямых отрезков (control points — соседние waypoints, границы клэмпятся, не wrap'ятся; прогресс сегмента считается по длине прямой хорды, не по истинной длине дуги — приближение). Без коллизий с solids (проходит сквозь геометрию) и без переноса игрока/пассажиров на движущейся платформе — не запрошено, вне рамок прохода. |
...
| `tilemap` | Tilemap | 1 | implemented | Grid layer: `tilesetAssetId?` (catalog tileset), `imageAssetId?` / `src?` (atlas), `tileWidth`/`tileHeight` (16), `columns` (1), `mapWidth`/`mapHeight`, `tiles` (row-major atlas indices, `-1` empty), `solidIndices` (`null` = all non-empty solid, `[]` = none, array = whitelist), `layer`. `TilemapBehavior.getSolidRects()` → per-cell AABBs; `collectSolidBlockers` expands them for PlayerMovement/movable/mount. `drawTiles` in Renderer; atlas preload via `collectImageSources`. No autotiling / brush UI. §7 Tier 3 v4.42.0. |
