...
| `stateMachineBehavior` | State Machine / Behavior | 1 | implemented | `defaultState` (string, имя начального состояния, дефолт — первый элемент `states`), `states` (JSON `[{name, movement, speed, waypoints, transitions}]`). AI-стейт-машина NPC (patrol/chase) — отдельный компонент от `spriteUiAnimation`'s Фаза F clip-state-machine (та переключает спрайт-клипы, эта двигает entity), но переиспользует тот же механизм переходов: каждый тик проверяет `transitions` текущего состояния по порядку (первый matching wins), условие — либо `{type:'distance', op, value}` (дистанция от entity до `scene.player`, единственная поддерживаемая цель chase/flee в этом проходе), либо `{var, op, value}` через общий `ConditionEvaluator.evalSpec` против `scene.eventGraphRuntime` (тот же формат условий, что Event Graph/Dialogue/Фаза F). `movement` управляет тик-движением состояния: `patrol` — ping-pong между `waypoints` (offsets от spawn, та же asset-local конвенция что `pathFollower`), `chase`/`flee` — прямое движение к/от `scene.player` на `speed` (дефолт 100), без `movement` (или `idle`) — нет движения. Без коллизий с solids (как `pathFollower`). Публичный `setState(name)` — duck-typed hook для будущего Event Graph action "SetAIState" (не подключён). |
