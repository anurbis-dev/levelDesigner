...
| `materialShaderPreset` | Material / Shader Preset | 1 | partial (Tier 1: `entity.materialPreset` → canvas filter) | No separate reusable catalog asset/editor form yet — `materialShaderPreset` itself isn't spawnable via `ProjectLoader.assetsById`/`SpawnObject` (Tier 2, see `prefab` row), only generic catalog assets are. Preset is a direct GameObject field (same convention as `color`/`imgSrc`, not a component `properties` bag): `materialPreset` (JSON `{blur?, brightness?, saturate?, hueRotate?, dropShadow?:{x?,y?,blur?,color?}}`). `Renderer._buildFilterString()` turns it into a canvas 2D CSS `filter` string, applied to every entity draw (image or fallback rect) in `Renderer._drawSingle`; resets to `'none'` for entities without a preset. Not gated behind a `volume` trigger zone — `volume` (arbitrary-shape visual-effect zone) is a separate, still-unimplemented §7 item. |
...
Источник: `COMPONENT_TYPES` в `src/constants/ComponentTypes.js`, 20 id. Статус по каждому — в
