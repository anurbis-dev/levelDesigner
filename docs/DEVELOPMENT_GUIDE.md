...
При изменении версии проекта необходимо обновить её в следующих местах:

1. **package.json** - основная версия проекта
2. **index.html** - заголовок страницы `<title>`
3. **src/core/LevelEditorRefactored.js** - статическая переменная `LevelEditor.VERSION`
4. **src/models/Level.js** - версия по умолчанию для новых уровней
5. **@version** комментарии в файлах CSS/JS

Версия автоматически логируется при запуске редактора и отображается в UI:

**Логирование в консоли:**
```javascript
// В консоли браузера появится:
// 🚀 Level Editor v2.0.0 - Utility Architecture
```

**Отображение в интерфейсе:**
- Версия показывается в правом нижнем углу панели ассетов
- Автоматически обновляется из `LevelEditor.VERSION`
- Стили: серый текст размера xs (`text-xs text-gray-500`)
