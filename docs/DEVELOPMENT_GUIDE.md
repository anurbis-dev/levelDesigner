...
export class LevelEditor {
    static VERSION = '2.0.0'; // ← ЕДИНЫЙ ИСТОЧНИК ИСТИНЫ
}
```

**Все остальные места получают версию динамически:**
- Console логирование
- UI отображение  
- Команда `version` в консоли
- Остальные места помечены как `dynamic` и не требуют обновления

**Для обновления версии:** измените только `LevelEditor.VERSION` - все остальное обновится автоматически.
