# API Reference - 2D Level Editor v3.4.1
...
- **Группы как объекты слоев** - группы могут иметь собственный layerId
- **Наследование вниз по иерархии** - вложенные объекты наследуют layerId от родительской группы
- **Приоритет собственного layerId** - объект использует свой layerId, если он установлен
- **Эффективный расчет layerId** - методы getEffectiveLayerId() для всех компонентов
- **Оптимизированная фильтрация** - рендеринг и выделение учитывают наследование layerId

**Новые методы:**
- `DetailsPanel.getEffectiveLayerId(obj, level)` - расчет эффективного layerId для отображения
- `RenderOperations.getEffectiveLayerId(obj)` - расчет для рендеринга
- `ObjectOperations.computeSelectableSet()` - обновлена логика выделения с учетом наследования
