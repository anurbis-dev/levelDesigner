...
### Refactored - Рефакторинг системы табов панели ассетов
...
- **AssetTabsManager.js** - новый класс с методами `render()`, `syncDefaultTab()`, `addFolderTab()`, `removeFolderTab()`, `handleTabClick()`
...
#### 📋 **Измененные файлы:**
- **`src/ui/AssetPanel.js`** - рефакторинг, делегирование логики табов в AssetTabsManager
- **`src/ui/AssetTabsManager.js`** - новый класс для управления табами (включая AssetTabContextMenu)
- **`styles/panels.css`** - добавлены CSS классы для drop overlay
- **`src/ui/FoldersPanel.js`** - улучшена обработка динамически добавленных ассетов
