...
- **Режим редактирования групп** - специальный режим для работы с объектами внутри групп
- **Alt+Drag функциональность** - перемещение объектов между группами с зажатым Alt
- **Дублирование объектов** - создание копий с сохранением относительных позиций
- **Префабы** - создание переиспользуемых шаблонов
- **Экспорт/Импорт** - сохранение и загрузка уровней в JSON формате
- **Отмена действий** - отмена любых текущих операций по Esc или ПКМ
...
└── tmp/              # Временные файлы
    └── duplicate_renderer_fixed.js # Рендерер дублированных объектов
```
...
3. Используйте панель ассетов для выбора объектов
4. Перетащите ассеты на canvas для размещения
5. Используйте правую панель для редактирования свойств
6. Сохраните уровень через меню Level → Save Level

## Рефакторинг архитектуры

Проект был рефакторен для улучшения читаемости и поддержки кода. Основной класс `LevelEditor` был разбит на модули:

### Модульная архитектура

- **EventHandlers** - управление всеми событиями (мышь, клавиатура, меню)
- **MouseHandlers** - обработка взаимодействий с мышью
- **ObjectOperations** - операции с объектами (создание, удаление, дублирование)
- **GroupOperations** - операции с группами (группировка, разгруппировка, редактирование)
- **RenderOperations** - все операции рендеринга
...
- **Переиспользование** - модули можно использовать в других частях
