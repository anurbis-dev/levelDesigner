...
- **Меню-структура** (новая в Phase 7):
  - Project-операции: New Project, Open Project..., Open Recent, [separator], Set Project Folder..., Clear Project Folder, [separator], Save Project, Save Project As...
...
- `newProject()` — создание нового проекта: очищает все открытые уровни, создаёт один пустой с `seeded history baseline` (один undo-шаг для начального состояния). Confirm если unsaved ИЛИ bound folder: "Starting a new project will close all currently open levels." + optional "Unsaved changes will be lost." + optional "The project folder will be cleared and Assets will reload from the default content folder." + "Continue?" (вместо N диалогов на каждый уровень; Edge Case 10/11). После `new Project()` вызывает `clearProjectFolder()` — drop active FSA bind (`workingDir` + localStorage name) и rescan served `./content/`. Вызов **после** `new Project()` (`fileName === null`) не трогает IDB `project:<file>` предыдущего проекта.
- `openProject()` — picker + read, затем `openProjectFromData(json, fileName)`.
- `openProjectFromData(json, fileName)` — открытие из JSON (диск или Open Recent): парсит all уровни ДО очистки текущих (Edge Case 3); replace-not-merge; пишет MRU; затем `editor.restoreProjectFolder(project.fileName)`.
- `saveProject()` / `_doSaveProject()` — async. При отсутствии pinned `project.fileName` имя из `project.name` через `_deriveFileNameFromProjectName()`; auto-имя пока `fileNameIsAuto`. Пишет через `FsaStore.writeWorkingDirFile(fileName)` в granted folder; иначе `FileUtils.downloadData`. После save — `FsaStore.bindProjectDirectory(fileName, workingDir handle)` + MRU.
...
- Меню: File → Open Recent (rebuild on hover) + Clear Recent. Open / Open Recent вызывают `restoreProjectFolder(fileName)`. После Open Recent, перед Save Project — **Set Project Folder...** (`set-project-folder` → `LevelEditor.setProjectFolder()` → pick + bind `project.fileName` + `reloadProjectContent()`) и **Clear Project Folder** (`clear-project-folder` → `LevelEditor.clearProjectFolder()`, drop active bind + unbind текущего проекта).
...
- **AssetViewRenderer** (Фаза 4.2) — рендеринг превью (grid/list/details), `render()` — one-line delegate из AssetPanel.render(); `renderPreviews` cache `renderKey` = viewMode|size|gap + per-asset `id:dirty:tmp:name:lastSaved` + selection; `invalidatePreviewCache()` сбрасывает ключ (после persist)
...
- **AssetDragDropController** (Фаза 4.5) — drag-out ассетов на канвас, external PNG file drop overlay, создание ассетов из файлов; sets `application/x-asset-ids`, effectAllowed `copyMove` (canvas drop still copies)
- **AssetItemActionsController** (Фаза 4.6) — контекстные меню (AssetContextMenu, AssetPanelContextMenu), `folderOps` (`AssetFolderOps`), delete-with-disk, клики по ассетам, open/show-in-explorer; RMB **Save Asset(s)** / **Save Asset(s) To...** → `AssetPanel.saveSelectedAssets` на всём выделении, если кликнутый ассет в нём
...
**Декомпозиция завершена** (Фаза 4): AssetPanel.js остаётся orchestration-слоем (constructor, init, destroy, setupEventListeners, setupAssetPanelHandlers, setupAssetEvents, updateContentVisibility, handleAssetWheel, handleDrop/createTemporaryAssetFromFile, saveSelectedAssets/_saveAssetsList/_markAssetPersisted/_refreshAfterAssetSave, handleAssetSave/handleAssetSaveChanges/handleAssetShowInExplorer, autoResizePanelHeight, showSaveSuccessMessage/showSaveErrorMessage/showErrorMessage, shouldShowUnsavedIndicator, плюс delegate-методы для контроллеров). Архитектура: 3099→1154 строк (62% сокращение).

**Save library assets**: `EventHandlers._saveAssetsOnCtrlS` (клавиша `editor.saveLevel` / `Ctrl+S`) — hover Assets dock/panel + nonempty `selectedAssets` → `AssetPanel.saveSelectedAssets()` (temp/no path → `handleAssetSave`, else `handleAssetSaveChanges`); hover Asset Editor (тот же probe `_assetPreviewForHotkey`, что F/A) + `editingAssetId` → save этого ассета; иначе `editor.saveLevel()`. File → Save всегда уровень. После записи: `_markAssetPersisted` (`hasUnsavedChanges=false`, `lastSaved`, clamp `lastModified`, `saveOriginalState`) и `_refreshAfterAssetSave` (`invalidatePreviewCache` + `assetsChanged`). Dirty-dot: `shouldShowUnsavedIndicator` = `isTemporary || hasUnsavedChanges || hasChangesFromOriginal()` (без `lastModified>lastSaved`).
...
- **Контекстные меню**: правый клик для дополнительных действий (AssetItemActionsController); asset RMB has no **Move to Folder**
- **Content tree RMB (v4.52.0)**: `FoldersContextMenu` — **New Folder** / **Delete Folder** (Delete скрыт на root); empty-area RMB в превью — **New Folder** (active folder)
- **Drag-move assets/folders (v4.54.0)**: click-drag asset thumbnail(s) onto a Content-tree folder (`FoldersPanel`) or an existing folder tab (`AssetTabsManager`) → `AssetFolderOps.moveAssetsByIds` (JSON + sibling PNG + manifest when project folder granted). Drag a folder onto another Content-tree folder → `moveFolders` (folder + all contents). Cannot move Content root; cannot drop a folder into itself/descendant. Dropping a folder onto the tabs strip still pins a tab (does not move). After move, `AssetPathRewriter.rewriteAll` remaps path-like strings (`path`, `imgSrc`, `properties.sourceFile`, `properties.levelSrc`, and any string that equals or is prefixed by the old content-relative path, including `./content/`, `content/`, `root/` forms). Component `*AssetId` fields stay. Open level objects/properties are rewritten the same way. Moved asset JSON is saved with stable `id`.
- **Folder / asset disk CRUD (v4.52.0–v4.54.0)**: `AssetFolderOps` — create/delete folders, `moveAssets`/`moveFolders`/delete assets (+ sibling PNG + manifest) when project folder granted; Del over folders tree (or over assets panel with non-root folder selected and no assets selected) → delete selected folder(s); cannot delete Content root
- **Project folder as library (v4.52.0 / v4.55.0)**: after **Set Project Folder**, Content tree shows FSA scan of that folder (replaces served `./content/` library). Open / Open Recent restores the folder last bound to that project file (`project:<basename>`). File → **Clear Project Folder** and Project Settings → Clear drop the active bind and unbind only the **open** project's key; **New Project** drops the active bind (default `./content/`) but keeps other projects' stored handles.
...
- **AssetManager**: управление библиотекой ассетов; `scanContentFolder()` (fetch `./content/`) или `scanFromFsa()` (v4.52.0 FSA tree → `ingestAssetJson`); `ingestAssetJson` классифицирует type через `resolveCatalogAssetType` (level-document JSON → `level`, `imgSrc=null` для non-image); кэширование изображений
  ...
  - **AssetTypes каталог** (`src/constants/AssetTypes.js`): 31 предопределённый тип ассетов в 6 категориях (Core: Camera, Actor, Image, **Level**, ImageAtlas, Volume, Player Start; Visual/Render: SpriteAnimationClip, Tileset, Tilemap, NineSliceSprite, FontTextStyle, ParticleEffect, MaterialShaderPreset, Light; Audio: SoundEffect, MusicTrack, AudioZone; Data/System: **Event Graph**, DialogueGraph, QuestObjective, ItemDefinition, InventorySchema, LocalizationTable, SaveSchema, InputMap; Navigation/AI: PathSpline, NavMesh, AIBehaviorPreset; Other: Prefab, SequenceCutscene). `type=level` — не placed object: drop на viewport открывает карту (`openLevelFromAsset`); `type=eventGraph` — drop применяет graph к текущему уровню. `Player Start` — auto-managed GameObject marker (ровно один на уровень). Хелперы: `getAssetTypeById(id)`, `getAssetTypesByCategory(categoryId)`, `getAssetCategoriesWithTypes()`. Классификация scan: `LevelAssetUtils.resolveCatalogAssetType` / `isLevelDocument`.
  - **ComponentTypes каталог** (`src/constants/ComponentTypes.js`): 30 типов компонентов (включая `playerStart` с `movementMode` + `crouchHeight`/`proneHeight`, `camera` с `followLerp`/`lookAhead`/`viewHeight`, `tilemap` с `tileKinds`/`tilesetAssetId`+`imageAssetId`, плюс sprite/audioZone/particleEffect/light/nineSliceSprite/fontTextStyle/volume/navMesh/sequenceCutscene) — editor-side metadata-стабы ({id, type, enabled, properties}); runtime в `src/engine/` (`docs/RUNTIME_SCHEMA.md`); `sprite` зарегистрирован как no-op `Behavior`. Хелперы: `getComponentTypeById(id)`, `createComponentStub(typeId)`
...
- **Файловые операции уровней с поддержкой multi-level сессий** (Фаза 5):
  ...
  - `openLevelFromAsset(asset)` (v4.53.0) — drop type=`level` на viewport: `resolveMapSrc` (только `properties.levelSrc`, не catalog `path`) → FSA `readText`, затем `contentUrl`; missing file → null. Map JSON → `openLevelFromData`; иначе empty `fileManager.createNewLevel()` + `asset.name`, `fileName` из `resolveLevelFileName`. Dblclick — Asset Editor.
  - `async saveLevel()` / `async saveLevelAs()` — await `fileManager.saveLevel`; per-session `session.fileName` (не глобальное `FileManager.currentFileName`). FSA content write → `content/maps/<file>` (или relative path) + manifest; иначе download. После save: `session.isDirty = false`; status «Level saved to project folder» если folder granted. Клавиатурный `Ctrl+S` (`editor.saveLevel`) сначала `EventHandlers._saveAssetsOnCtrlS`; File → Save вызывает `saveLevel` напрямую.
...
### FsaStore / FsaContentWriter / FsaContentFs / AssetPathRewriter (v4.51.0–v4.55.0)
**Файлы**: `src/utils/FsaStore.js`, `src/utils/FsaContentWriter.js`, `src/utils/FsaContentFs.js`, `src/utils/AssetPathRewriter.js`, `src/ui/AssetFolderOps.js`, `src/ui/FoldersContextMenu.js`
- **FsaStore** (static): feature-detect `showOpenFilePicker`+`showSaveFilePicker`+`showDirectoryPicker`. Persist `FileSystemDirectoryHandle` в IndexedDB `levelDesignerFsaHandles` / store `handles`: key `workingDir` (active bind) **и** `project:<basename>` (например `project:My Game.json`). Имя папки — localStorage `levelDesignerFsaWorkingDirName` (machine-local, **не** Project.toJSON). API: `isSupported()`, `getWorkingDirectoryName()`, `pickWorkingDirectory()` (`showDirectoryPicker({ mode: 'readwrite' })`), `clearWorkingDirectory()`, `getWorkingDirectoryHandle()`, `projectDirKey(fileName)`, `bindProjectDirectory(fileName, handle)`, `unbindProjectDirectory(fileName)`, `getProjectDirectoryHandle(fileName)`, `activateDirectoryHandle(handle)`, `restoreProjectDirectory(fileName)`, `verifyPermission(handle, mode)`, `getContentDirectoryHandle()` (subdir `content/` если есть, иначе сам folder), `writeWorkingDirFile(rel, data)`, `writeContentFile(rel, data)`, `ensureManifestEntry(rel, assetManager?)` (обновляет `content/manifest.json` `files[]` + `structure`), `toContentRelativePath(path)` (strips `root/`/`./content/`/`content/`), `toLevelRelativePath(fileName)` (bare name → `maps/<name>`).
- **FsaContentWriter** (static): `assetToDiskJson(asset)` (persistable JSON, `imgSrc` = sibling basename, drop `path`/data-URL imgSrc); `saveAsset(asset, {updateManifest, assetManager})` / `saveNewAsset(asset, assetManager)` — write JSON + PNG sibling when imgSrc is data URL. `AssetPanel.saveSelectedAssets` → `handleAssetSave` (temp) / `handleAssetSaveChanges` (existing) пробуют FSA first; picker/`saveDataDirectly` только если null. После успешной записи — `_markAssetPersisted` + `invalidatePreviewCache`/`assetsChanged`.
- **FsaContentFs** (static, v4.52.0–v4.54.0): directory/file ops on content root. `ensureDefaultContentLayout()` — if pick is not already a content root (`manifest.json` or `assets`/`maps`/`graphs`), creates nested `content/`; ensures default dirs + manifest. `scanContentTree()` → `{ structure, files }`. CRUD: `createDirectory`/`deletePath`/`moveFile` (copies Blob — PNG-safe)/`moveDirectory`/`readText`/`readBlob`/`resolveImageSrc` (blob URL from FSA, else `./content/` fallback)/`updateManifest(mutator)`. Path helpers: `uiFolderToContentRel`/`contentRelToUiFolder`/`sanitizeFolderName`/`addStructureFolder`/`removeStructureNode`/`flattenFolderRels`/`isUnderPath`/`folderExists`/`moveStructureNode`. Internals: `_getChildHandle`/`_copyTree`.
- **AssetPathRewriter** (static, v4.54.0): `rewriteString` / `rewriteInPlace` / `rewriteAll(editor, remaps)`. Remaps path-like strings after a move (`path`/`imgSrc`/`sourceFile`/`levelSrc` and any string equal to or prefixed by the old content-relative path, including `./content/`, `content/`, `root/` forms). Component `*AssetId` refs stay. Rewrites catalog assets + the open level (and other `levelsManager` sessions).
- **LevelEditor project-folder bind (v4.52.0 / v4.55.0)**: `setProjectFolder()` = pick + `bindProjectDirectory(project.fileName)` если `fileName` есть + `reloadProjectContent()`. `restoreProjectFolder(fileName)` (Open / Recent) = `FsaStore.restoreProjectDirectory` + `reloadProjectContent()`; нет handle → `clearWorkingDirectory` + `scanContentFolder` (не оставлять папку предыдущего проекта). `reloadProjectContent({onlyIfGranted, skipFetchFallback})` ensure layout → `assetManager.scanFromFsa()` → refresh FoldersPanel. `clearProjectFolder()` drops `workingDir` + `unbindProjectDirectory` для **текущего** `project.fileName` и rescans served `./content/` (File → **Clear Project Folder**, Project Settings → Clear). `newProject()` вызывает его **после** `new Project()`, поэтому ключ предыдущего проекта в IDB остаётся.
- **AssetFolderOps** (v4.52.0–v4.54.0): create/delete folders; `moveAssets`/`moveAssetsByIds`/`moveFolder`/`moveFolders`/delete assets with disk+manifest when folder granted. No `buildMoveMenuItems`.
...
2. **FSA path (v4.52.0)**: `LevelEditor.reloadProjectContent()` → `FsaContentFs.ensureDefaultContentLayout()` → `AssetManager.scanFromFsa()` (`FsaContentFs.scanContentTree` + `resolveImageSrc` + `ingestAssetJson` → `LevelAssetUtils.resolveCatalogAssetType`; `LEDGE.json` / `maps/ledge.json` → `type=level`)
