...
- **UI customize key (Shift)**: `isDockCustomizeKey` — only while Shift held: move/split panels, self-drop clone, detach to floating (Shift+drag header gap to empty space — no leaf-header detach icon, DK-ICO), floating snap/ungroup, drop-zone & snap highlights. Floating free-move position + corner resize work without Shift; resize grip shows only when pointer is in the window’s bottom band.
