# Архитектура Level Editor v3.52.5
...
- **Исправления панелей и табов** - унифицированная логика создания/удаления панелей, исправлена бесконечная рекурсия, правильное обрезание табов
- **Новая архитектура с разделением слоёв** - канва на нижнем слое, интерфейс на верхнем, прозрачность событий, оптимизация CSS
- **Унифицированная система управления панелями** - централизованное управление сворачиванием/разворачиванием всех панелей через togglePanelCollapse, привязка позиционирования к методу сворачивания
- **Настройка цвета разделителей** - CSS переменная `--ui-resizer-color` для централизованного управления цветом всех разделителей панелей, интеграция с системой настроек
...
**Файлы**: `src/ui/BaseDialog.js`, `src/ui/panel-structures/DialogStructures.js`, `src/utils/DialogSizeManager.js`
- **BaseDialog** - единый базовый класс для всех всплывающих окон
- **Фиксированная высота** - `calc(100vh - 4rem)` для всех диалогов
- **Динамическая ширина** - автоматический расчет по самому широкому контенту
- **Плавная инициализация** - `visibility: hidden` до применения финальных размеров
- **DialogStructures** - предопределенные конфигурации для разных типов диалогов
- **DialogSizeManager** - утилита для расчета оптимальных размеров
- **Мобильная адаптация** - автоматическая интеграция с MobileInterfaceManager
- **Защита от перезаписи** - CSS переменные и специфичные селекторы для предотвращения конфликтов
