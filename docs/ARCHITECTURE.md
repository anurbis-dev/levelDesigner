...
- **Файловые операции уровней с поддержкой multi-level сессий** (Фаза 5):
  ...
  - `openLevelFromAsset(asset)` (v4.53.0) — drop type=`level` на viewport: `LevelAssetUtils.resolveLevelSrc` + `contentUrl` + fetch + `isLevelDocument` → `openLevelFromData`. Dblclick ассета по-прежнему открывает Asset Editor.
