...
- **ViewportViewManager**: N viewport leaves, each with canvas + pose + type filters + camera source + optional `displayOptions`.
  ...
  - **VP-HK**: `getViewUnderCursor()` / `viewFromClientPoint` — hotkeys F/A/Grid/Boundaries/Collisions/Parallax (and jump-to-camera) target the leaf under the cursor; `getDisplayFlag` / `toggleDisplayFlag` materialize per-view overrides (`showGrid`, `objectBoundaries`, `objectCollisions`, `parallax`); primary also syncs global state for menu. Focus (F/A) writes camera via `updateCamera(leafId)`.
  - **VP-TB**: each secondary viewport leaf mounts its own `Toolbar` copy (`DockPanelFactory.createViewportCopy` + `editor.viewportToolbars`); View toggles (Grid/Parallax/Boundaries/Collisions) and Focus/Focus All apply to the **paired** leaf via `getPairedView()` → `toggleDisplayFlag` / `focusOnSelection(view)`; File/Edit/Group/Play/Snap remain global; primary `#toolbar-container` uses `getPrimaryView()` the same way. `setDisplayFlag` refreshes chrome + `refreshViewportToolbars`.
- **ViewportLeafChrome**: leaf header icons — camera source menu (English UI) + per-view type filter via shared `TypeFilterMenu` + **VP-EYE** eye icon (per-view `showGrid`/`objectBoundaries`/`objectCollisions`/`parallax` checklist, `getDisplayFlag`/`toggleDisplayFlag`); hover-switch between sibling menus after first open (main-menu style).
