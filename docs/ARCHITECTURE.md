...
- Меню: File → Open Recent (rebuild on hover) + Clear Recent; после Open Recent, перед Save Project — **Set Project Folder...** (`set-project-folder` → `LevelEditor.setProjectFolder()` → pick + `reloadProjectContent()`) и **Clear Project Folder** (`clear-project-folder` → `LevelEditor.clearProjectFolder()`, тот же reset без нового проекта).
...
- **Project folder as library (v4.52.0 / v4.54.1)**: after **Set Project Folder**, Content tree shows FSA scan of that folder (replaces served `./content/` library); File → **Clear Project Folder**, Project Settings → Clear, and **New Project** revert to fetch `./content/`
...
### FsaStore / FsaContentWriter / FsaContentFs / AssetPathRewriter (v4.51.0–v4.54.0)
...
- **LevelEditor project-folder bind (v4.52.0 / v4.54.1)**: `setProjectFolder()` = pick + `reloadProjectContent()`; `reloadProjectContent({onlyIfGranted, skipFetchFallback})` ensure layout → `assetManager.scanFromFsa()` → refresh FoldersPanel; `clearProjectFolder()` clears handle and rescans served `./content/` (File → **Clear Project Folder**, Project Settings → Clear; `newProject()` also calls it).
