...
- **ViewportViewManager**: N peer viewport leaves (VP-EQ), each with canvas + `localCamera` + type filters + camera source + **owned** `displayOptions`.
  ...
  - **C1 camera view frame**: `RenderOperations.drawCameraViewFrames` draws a dashed world frustum for each camera object (center = object center, size = design res / `properties.zoom`, default 1920×1080 or `properties.resolutionWidth/Height`). Skipped for the camera currently driving that viewport. Stroke via `strokeFrame` + center cross; selected camera uses stronger cyan.
