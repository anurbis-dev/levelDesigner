...
- Сохраняет результат в файл `tmp/gitUtils/git_logs.txt`
- Выводит результат в консоль
- Обрабатывает ошибки
- Цветной вывод для лучшей читаемости
- Автоматически переходит в папку `tmp/gitUtils/` при выполнении
...
2. **Получение логов (безопасно):**
   ```bash
   # Используйте батники из tmp/gitUtils/
   tmp\gitUtils\get_git_logs.ps1 10
   # или
   tmp\gitUtils\get_git_logs.bat 10
   ```
...
```bash
# Безопасное получение логов (используйте батники из tmp/gitUtils/)
tmp\gitUtils\get_git_logs.ps1 10
tmp\gitUtils\get_git_logs.bat 10
...
**Важно:** Все батники автоматически переходят в папку `tmp/gitUtils/` при выполнении, поэтому все создаваемые файлы (включая `git_logs.txt`) сохраняются в этой папке, а не в корне проекта.


## 🚀 Быстрый старт

### Для агента AI:
1. Используйте `tmp\gitUtils\get_git_logs.ps1 10` для получения логов
2. Используйте `tmp\gitUtils\git_status.bat` для проверки статуса
...
3. Используйте прямые пути к `tmp/gitUtils/` для всех git операций
