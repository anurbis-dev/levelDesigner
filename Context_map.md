...
- **Asset Editor**: `src/ui/asset-editor/*` — open via `LevelEditor.showActorPropertiesPanel(asset)` (dblclick thumbnail); `stateManager.editingAssetId` / `editingComponentId`; live edit via `assetManager.updateAsset` (no Apply/Cancel modal)
