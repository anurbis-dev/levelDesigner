# Context Map - Level Designer v4.33.0 (Phase A done; Phase B dock B0–B5 done; §7 backlog 12/12 done: camera/pickup/damageHealth/movablePushable/mountableVehicleSeat/pathFollower/spawner/stateMachineBehavior/checkpointSavePoint/climbableLadder/conveyorZiplineJumpPadPortal/variableModifier/destructibleContainer; §7 Tier 1 all 4 done: soundEffect (PlaySound action), pathSpline (smooth Catmull-Rom), aiBehaviorPreset (aiPreset shorthand), materialShaderPreset (materialPreset field + filter string); §7 Tier 2 real assetRegistry (ProjectExporter embeds assets in manifest, ProjectLoader populates assetsById Map from manifest.assets, Scene.assetsById attaches registry per-level, EntityFactory.fromAssetData() spawns prefabs/composites at runtime, SpawnObject action registered); HUD Canvas runtime + data model (CanvasHudRenderer/CanvasHudBinding, Level.canvases, Scene.canvases Map, camera.canvasIds component property, no editor UI yet), v4.33.0)
...
- **Фаза 1 MVP-ядро** (`src/engine/` самодостаточен, ноль импортов из `src/constants|core|managers|models|ui|utils|widgets`):
  ...
  - `src/engine/Scene.js` - уровень рантайма (entities, layers, settings, camera, assetsById Map), методы `getLayersSorted/getLayerById/getVisibleLayerIds()`
  ...
  - **Фаза 2 MVP-поведения** (`src/engine/behaviors/` дочерние классы, `src/engine/BehaviorRegistry.js` реестр):
    ...
    - `src/engine/behaviors/registerDefaultBehaviors.js` - регистрирует  в `BehaviorRegistry`
  ...
  - **Фаза D Event Graph MVP** (`src/engine/eventgraph/`, Фаза D LOGIC_SYSTEMS_PLAN):
    ...
    - `src/engine/eventgraph/registerDefaultEventGraphNodes.js` - MVP-словарь узлов (Entry: OnStart/OnTick/OnCollisionEnter/OnCollisionExit/OnInteract/OnTimer/OnCustomEvent/OnDialogueEnded; Conditions: Compare/And/Or/Not; Actions: SetVariable/SetComponentEnabled/Teleport/DestroyObject/EmitCustomEvent/StartDialogue/PlayAnimation/PlaySound/SpawnObject); SpawnObject новый action (params: {assetId, x?, y?, layerId?}) создаёт GameObject из `scene.assetsById` и добавляет в `scene.entities`
  ...
  - **HUD Canvas rendering** (v4.33.0):
    ...
    - `src/constants/ComponentPropertySchema.js` — `camera` схема добавила новое поле: `canvasIds` (kind 'stringList', default `[]`) — HUD Canvas ids which active camera shows
...
Версия в одном месте: `src/core/LevelEditor.js` → `static VERSION = '4.33.0'` (Phase A refactor + Phase B dock B0–B5 + §7 backlog all 12/12 done: camera/pickup/damageHealth/movablePushable/mountableVehicleSeat/pathFollower/spawner/stateMachineBehavior/checkpointSavePoint/climbableLadder/conveyorZiplineJumpPadPortal/variableModifier/destructibleContainer; §7 Tier 1 all 4 done: soundEffect/pathSpline/aiBehaviorPreset/materialShaderPreset; §7 Tier 2 real assetRegistry: ProjectExporter embeds assets[] in manifest, ProjectLoader populates assetsById Map from manifest.assets, Scene.assetsById per-level, EntityFactory.fromAssetData() for prefab/composite spawning, SpawnObject action; HUD Canvas runtime + data model: Level.canvases[], Scene.canvases Map, camera.canvasIds component property, CanvasHudRenderer/CanvasHudBinding helpers, no editor UI yet; layout = `editor.dockManager`)
