...
- `src/ui/canvas-hud/CanvasHudPanel.js` - dock panel для уровня-широкого каталога HUD Canvases (3-column list/list/form layout, mirrors DialoguesPanel/ItemsPanel); операции CRUD через HistoryManager `setCanvasesProvider()`; обновляет стейт-ключ `canvasesRevision` для синхронизации между копиями
