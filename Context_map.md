...
Версия в одном месте: `src/core/LevelEditor.js` → `static VERSION = '4.32.0'` (Phase A refactor + Phase B dock B0–B5 + §7 backlog all 12/12 done: camera/pickup/damageHealth/movablePushable/mountableVehicleSeat/pathFollower/spawner/stateMachineBehavior/checkpointSavePoint/climbableLadder/conveyorZiplineJumpPadPortal/variableModifier/destructibleContainer; §7 Tier 1 all 4 done: soundEffect/pathSpline/aiBehaviorPreset/materialShaderPreset; §7 Tier 2 real assetRegistry: ProjectExporter embeds assets[] in manifest, ProjectLoader populates assetsById Map from manifest.assets, Scene.assetsById per-level, EntityFactory.fromAssetData() for prefab/composite spawning, SpawnObject action; layout = `editor.dockManager`)
