...
- `src/constants/ComponentTypes.js` - каталог 30 типов компонентов (Collider, Trigger, Interactable, PlayerStart, Camera и т.д.); `createComponentStub()`
...
- `docs/RUNTIME_SCHEMA.md` - контракт runtime-схемы: per-type `schemaVersion`/`properties` для 29 asset-типов и 30 component-типов, плюс раздел `## Versioning` про `Level.meta.version` (semver уровня, отдельная ось от `LevelEditor.VERSION`)
- `docs/LEDGE_PORT_PLAN.md` - план переноса прототипа LEDGE; уровень `content/maps/ledge.json`, арт `content/assets/ledge/`, сборка `node scripts/build-ledge-level.mjs`
...
- **Фаза 1 MVP-ядро** (`src/engine/` самодостаточен, ноль импортов из `src/constants|core|managers|models|ui|utils|widgets`):
  ...
  - **Фаза 2 MVP-поведения** (`src/engine/behaviors/` дочерние классы, `src/engine/BehaviorRegistry.js` реестр):
    ...
    - Обновлены: `Entity.js` (`behaviors: []`), `EntityFactory.js` (компоненты → behaviors через реестр), `Scene.js` (`getAllEntities()`, `findPlayerStartEntity()`, `getPlayerStart()`, `spawnPlayer(speed)`, `_createPlayer()` копирует `movementMode`/`bodyWidth`/`bodyHeight`/`speed`/`platformer`/`imgSrc` + `damageHealth`/`spriteUiAnimation` с маркера, `respawnPlayer(speed)`, `checkpointPosition`/`activeCheckpoint`/`_playerStartMarker` кеш, `heldEntity`, `findCameraEntity()`, `hideCameraMarker()` — кеширует `scene.cameraEntity`), `GameEngine.js` (регистрация + `_update(dt)`: `input.beginFrame` → behaviors → eventGraph → `respawnPlayer()` если `scene.player` falsy → `input.endFrame`; `tick` рисует `drawFade(scene.fade)`)
