...
- **Asset Editor**: `src/ui/asset-editor/*` + `constants/ComponentPropertySchema.js` — open via `showActorPropertiesPanel(asset)`; `editingAssetId` / `editingComponentId`; live `updateAsset`; collider/trigger `shape` box|circle|freeform + `color` + freeform point edit; sprite/interactable schemas; preview overlays. Layout: `panels.dock.assetEditorLayout` (rel pos/size + tree) survives close
