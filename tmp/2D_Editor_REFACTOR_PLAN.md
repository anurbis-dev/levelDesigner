...
| 7. Долгосрочные guardrails | ✅ done | `CONTRIBUTING.md` создан (правило про Controller/Operations перед новым файлом), `npm run check:dedup` (`npx jscpd`) добавлен, overrides в `scripts/check-file-size.js` сверены — `npm run check:size` подтвердил 41 актуальный override, stale нет |
...
| 7. Долгосрочные правила (закрыта) | — | Не откатиться назад через месяц | Фаза 1 (закрыта), Фаза 4 (закрыта) |
