...
- **Event Graph вкладка (asset-scope UI)**: отложена — runtime Event Graph (Фаза D) есть, canvas-
  виджет графа в редакторе ещё нет. Следующий шаг после C: UI Event Graph (level + asset scope).
